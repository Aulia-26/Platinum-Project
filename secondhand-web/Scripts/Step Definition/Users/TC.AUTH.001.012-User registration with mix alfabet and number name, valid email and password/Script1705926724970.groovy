...
WebUI.callTestCase(findTestCase('Test Templates/Registration/Registration-Master'), [('uname') : getRandomStringSymbol(8)
        , ('uemail') : generateRandomEmail(), ('upassword') : getRandomString(8)], FailureHandling.STOP_ON_FAILURE)

WebUI.verifyElementPresent(findTestObject('Homepage/btn_Masuk'), 5)

def generateRandomString(int length) {
    def characters = (('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789') as char[]).toList()

    def randomString = new StringBuilder()

    (1..length).each({ 
            def randomIndex = new Random().nextInt(characters.size())
...
def getRandomStringSymbol(int length) {
    def characters = (('abcdefghijklmnopqrstuvwxyz0123456789`~!@#$%^&*()_+{}|[]:">?.,./:-=') as char[]).toList()
